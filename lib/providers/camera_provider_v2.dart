...
class CameraProvider extends ChangeNotifier {
  ...
  bool _isSendingRemoteFrame = false;
  DateTime? _lastFrameAt;
  DateTime? _lastRemoteFrameAt;
  ...
  static const Duration _remoteFrameInterval = Duration(milliseconds: 120);
  static const int _remoteFrameWidth = 320;
  ...
  Future<void> stopCamera({bool preserveMode = false, bool disconnectWs = true}) async {
    ...
    try {
      ...
      _isSendingRemoteFrame = false;
      _lastFrameAt = null;
      _lastRemoteFrameAt = null;
  ...
  Future<bool> startRemoteCamera(String serverUrl) async {
    ...
    try {
      ...
      _isCameraActive = true;
      _isRecordingSegment = false;
      _setWaitingState('Mengirim frame ke server...');
      notifyListeners();
      if (_controller!.value.isStreamingImages) {
        await _controller!.stopImageStream();
      }
      await _controller!.startImageStream(_onRemoteFrame);
      return true;
    } catch (e) {
  ...
  Future<void> _onRemoteFrame(CameraImage image) async {
    if (!_isCameraActive || !_isInitialized || !_useRemoteServer) return;
    if (_isSendingRemoteFrame) return;

    final DateTime now = DateTime.now();
    if (_lastRemoteFrameAt != null &&
        now.difference(_lastRemoteFrameAt!) < _remoteFrameInterval) {
      return;
    }
    _lastRemoteFrameAt = now;
    _isSendingRemoteFrame = true;

    try {
      if (_wsChannel == null) return;
      final _EncodedFrame? encoded = _encodeCameraImage(image);
      if (encoded == null || encoded.bytes.isEmpty) {
        debugPrint('#remote empty frame, skip send');
        return;
      }
      final String b64 = base64Encode(encoded.bytes);
      ...
      debugPrint('Error sending remote frame: $e');
    } finally {
      _isSendingRemoteFrame = false;
    }
  }

  _EncodedFrame? _encodeCameraImage(CameraImage image) {
    try {
      final int width = image.width;
      final int height = image.height;
      final Uint8List rgb = _convertCameraImageToRgb(image);
      ...
      if (_remoteFrameWidth > 0 && base.width > _remoteFrameWidth) {
        base = img.copyResize(base, width: _remoteFrameWidth);
      }
      final bytes = Uint8List.fromList(img.encodeJpg(base, quality: 70));
      return _EncodedFrame(bytes, base.width, base.height);
    } catch (e) {
      debugPrint('Failed to encode frame: $e');
      return null;
    }
  }
...
class _EncodedFrame {
  ...
  const _EncodedFrame(this.bytes, this.width, this.height);
}
